...
# Run disko using the substituted SELF_OUT_PATH variable.
# Note the quoted substitution to satisfy shellcheck (SC2086).
sudo nix --experimental-features "nix-command flakes cgroups" run \
  github:nix-community/disko/latest -- --mode "$MODE" "${SELF_OUT_PATH}/templates/zfsonix.nix" \
